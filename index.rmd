...
1. Opinion homophily within couples. 
2. Political polarization within society. 
...
Political polarization between couples may be the result of: 
...
set.seed(98712)
data2 <- rmvnorm(n = 1000, mean=c(2,3), sigma=matrix(c(4,2,2,3),ncol=2))
...
```{r, fig.cap="Distribution of opinions within population"}
p1
```
...
```{r, fig.cap="Distribution of mean opinions within couples"}
p1 + p2
```
...
```{r, fig.cap="Distribution of mean opinions within couples, after shared life experiences"}
p2 + p3
```
...
```{r, fig.cap="Distribution of opinions within couples"}
p1 + p4
```
...
But what happens at the couple level? 

```{r, echo=FALSE, fig.cap="No change"}
library(ggplot2)
time <- rep(c(1:10),2)
time
...
```{r, echo=FALSE, fig.cap="Common trend"}
library(ggplot2)
time <- rep(c(1:10),2)
couple <- rep(c("man", "woman"), each=10)
opinion1 <- 4 + .5*c(1:10)
...
```{r, echo=FALSE, fig.cap="Convergence"}
library(ggplot2)
time <- rep(c(1:10),2)
...
```{r, echo=FALSE, fig.cap="Common Trend and Shocks"}
library(ggplot2)
time <- rep(c(1:10),2)
couple <- rep(c("man", "woman"), each=10)
opinion1 <- 4 + .5*c(1:10)
...
```{r, echo=FALSE, fig.cap="Convergence and shocks"}
library(ggplot2)
time <- rep(c(1:10),2)
