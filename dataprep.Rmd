...
file_names=as.list(dir(path="data-raw/leisure_integ_data/", pattern = '*.sav')) #TJ I added DOCS/ as the data-raw exists in the docs dir. 
...
#import data
MyData <- read_sav('data-raw/avars/avars_201812_EN_1.0p.sav')
...
liss_subset <- liss_wide %>%
  select(nomem_encr, nohouse_encr.1, nohouse_encr.2, nohouse_encr.3, nohouse_encr.4, nohouse_encr.5, nohouse_encr.6, nohouse_encr.7, nohouse_encr.8,
         ...
         positie.10, positie.11, euthanasia.1, euthanasia.2, euthanasia.3, euthanasia.4, euthanasia.5, euthanasia.6, euthanasia.7, euthanasia.8, euthanasia.9,
         euthanasia.10, euthanasia.11, income_diff.1, income_diff.2, income_diff.3, income_diff.4, income_diff.5, income_diff.6, income_diff.7, income_diff.8, income_diff.9,
         income_diff.10, income_diff.11, )
```
...
liss_subset_long <- reshape(as.data.frame(liss_subset),
                            direction = 'long',
                            varying = names(liss_subset[,2:89]),
                            idvar = 'nomem_encr',
                            times = 1:11)
...
#Then I will 
partner_function <- function (x) { #x <- partner_list[[1]]
  ...
  #create a second gesclacht variable.
  f <- 
...
The *partner_dataprepped.Rdata* consists of the prepped data for the partner paper, in long and wide format. For now, I have only included a selection of variables. The suffices .r and .p denote respondent and partner, .1 etc denotes the wave in the wide format. 
