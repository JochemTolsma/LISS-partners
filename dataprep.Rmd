...
Ik heb het script aangepast. Ik heb geprobeerd om jou structuur aan te houden. Daarnaast heb ik nu zoveel mogelijk tidyverse gebruikt. Hier en daar wel gewoon base r, omdat het me nu niet even lukt om het aan te passen. Maar dat wil ik op termijn wel aanpassen. Ik zal hier nog een lijst maken met de files die ik heb gebruikt en ik zal de codeboeken er bij zetten. Daarnaast zal ik ook nog de procedure van het alter identificeren uitleggen. Maar dit komt later, nu kan je in ieder geval in de vakantie spelen. 

Laat maar weten als je iets tegenkomt. Hier en daar staan nog wat vragen onder #TJ. 

Okee thijmen. 
hierbij wat reacties:
- TJ I added DOCS/ as the data-raw exists in the docs dir.: colorize("Nee niet doen!", "red"). de 'docs' folder is voor de website en staat ook in de gitignore. niet via deze dir werken dus.  
...
file_names=as.list(dir(path="docs/data-raw/pol_data/", pattern = '*.sav'))
liss_files <- vector(mode = "list", length = 11)

#loop to load all the data in one list.
#name them. 
for (i in 1:length(file_names)) {
  liss_files[[i]] <- read_sav(paste("docs/data-raw/pol_data/",file_names[[i]], sep=""))
} 
...
#import the data files.
file_names=as.list(dir(path="docs/data-raw/alter_data/", pattern = '*.sav'))
liss_names_alters <- vector(mode = "list", length = 11)

#loop to load all the data in one list.
#name them. 
for (i in 1:length(file_names)) {
  liss_names_alters[[i]] <- read_sav(paste("docs/data-raw/alter_data/",file_names[[i]], sep=""))
} 
...
#import data files.
file_names=as.list(dir(path="docs/data-raw/avars/", pattern = '*.sav'))
liss_files <- vector(mode = "list", length = 11)

#import the files in a for loop.
for (i in 1:length(file_names)) {
  liss_files[[i]] <- read_sav(paste("docs/data-raw/avars/",file_names[[i]], sep=""))
} 
...
liss_subset <- subset1 %>%
  ...
  mutate(na_pol_val = sum(is.na(c(cv08a_m, cv09b_m, cv10c_m, cv11d_m, cv12e_m, cv13f_m, cv14g_m,
         cv17i_m, cv18j_m1, cv18j_m2, cv18j_m3, cv19k_m1, cv19k_m2, cv19k_m3)))) %>%
...
liss_wide <- cbind(liss_pol, liss_avars, liss_alters ,liss_alter_id, liss_origin)
...
#TJ: I will do this another time. Now I do not have the time to do this. 

#pol discussion. 
#we need to reverse recode political discussion.
list <- liss_wide[,grep(names(liss_wide), pattern = "poltalk_alter", value = T)]
...
liss_wide[,grep(names(liss_wide), pattern = "poltalk_alter", value = T)] <- list2
...
list <- liss_wide[,grep(names(liss_wide), pattern = "educ_alter", value = T)]
...
#replace original variables with the reverse coded variable. 
liss_wide[,grep(names(liss_wide), pattern = "educ_alter", value = T)] <- list2
...
#create a long file.
liss_long <- reshape(liss_wide, direction = "long",
                     varying = names(liss_wide[,2:1332]),
                     idvar = "nomem_encr",
                     timevar = "survey_wave",
                     times = c(1:11))
...
Okay, once we agree on the procedure, we can delete this section. TJ: Agree, so this section can be deleted. 
```{r procedure explanation}
#import data
MyData <- read_sav('docs/data-raw/avars_201912_EN_1.0p.sav')
...
MyData <- MyData %>%
  filter(positie < 4) #JT changed this to 4! #TJ: fine by me
...
#merge the two df's
MyData <- right_join(MyData, list_hh, by  = 'nohouse_encr')
...
test_2 <- reshape(as.data.frame(test),
                  direction = 'wide',
...
test3 <- 
  test3 %>% mutate(geslacht2=ifelse(geslacht==1,"m", "f" ))
...
#TJ: prima laten we deze procedure gebruiken.
...
#TJ: I need to change this code into tidy code. I will do this at a later date  
...
We can start with the data preperation.

```{r prep of the partner data 2}
#reshape into a wide file.
liss_subset_long <- reshape(as.data.frame(liss_subset),
                            direction = 'long',
...
partner_function <- function (x) {
  ...
  a <- x %>%
    filter(!is.na(positie) & positie < 4)
  #count the members of the hh
  b <- a %>% 
    count(nohouse_encr)
  #merge the two datasources
  c <- right_join(a, b, by = "nohouse_encr")
  #select on 2.
  d <- c %>%
    filter(n == 2)
  
  #selection on hetero couples.  
  e <- d %>% 
  arrange(nohouse_encr, geslacht) %>% #ordenen op geslacht
  group_by(nohouse_encr) %>%  #groeperen op huishouden
  summarise(hetero=(sum(geslacht)==3)) %>% #bepaal of homo/heterostel
  right_join(d) %>% #koppel weer aan originele data
  arrange(nohouse_encr, geslacht) %>% #opnieuw ordenen
  filter(hetero==TRUE) # en de homostellen verwijderen. 
  
  #length of df, which we need for rep function.
  f <- 
  e %>% mutate(geslacht2=ifelse(geslacht==1,"m", "f" ))
  
  #reshape into wide file.
  g <- reshape(as.data.frame(f),
          direction = 'wide',
          v.names = names(f[,c(3,5:9,11)]),
          idvar = 'nohouse_encr',
          timevar = 'geslacht2')
...
partner_df_wide <- reshape(partner_df_long,
                           direction = 'wide',
                           v.names = names(partner_df_long[,2,4:15]),
                           idvar = 'nohouse_encr',
                           timevar = 'time')
